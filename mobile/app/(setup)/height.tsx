...
export default function HeightScreen() {
  ...
  const handleScroll = useCallback((event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    if (heights[clamped] !== selectedHeight) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.numbersRow}>
        {[-2, -1, 0, 1, 2].map((offset) => {
          const num = selectedHeight + offset;
          if (num < MIN_HEIGHT || num > MAX_HEIGHT) return <Text key={offset} style={styles.numberText}> </Text>;
      ...
      <View style={styles.rulerContainer}>
        ...
        <Animated.FlatList
          ...
          initialScrollIndex={165 - MIN_HEIGHT}
      ...
      <View style={styles.valueSection}>
        ...
        <View style={styles.valueRow}>
          <Text style={styles.bigNumber}>{selectedHeight}</Text>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          useSetupStore.getState().updateData({ height: selectedHeight });
          router.push('/(setup)/goal' as any);
        }}
      >
...
const styles = StyleSheet.create({
  ...
  numbersRow: {
    ...
    marginTop: 40, // push down a bit since there is no unit toggle here
  },
